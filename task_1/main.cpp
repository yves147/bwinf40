...
int main()
{
    ...
    // initially no cars have to be moved and all cars can be moved as far as possible
    std::vector<std::vector<int>>
        left_shift(cars_amount, std::vector<int>(cars_amount, 0));
    ...
    for(auto q_car: q_cars) {
        ...
        if(left2 >= 0 && possible_moves[left1] >= 2 && possible_moves[left2] >= 1) {
            ...
            if(possible_moves[left2] == 1) {
                update_row_dir(left_shift[first], left_shift[left2]);
            }
            else if(possible_moves[left2] == 2) {
                update_row_red(left_shift[first], left_shift[left2]);
            }
            possible_moves[first]      = 2;
            possible_moves[second]     = 2;
            left_shift[first][first]   = 2;
            left_shift[first][second]  = 2;
            left_shift[second][first]  = 2;
            left_shift[second][second] = 2;
        }
        else if(left1 >= 0 && possible_moves[left1] >= 1) {
            if(possible_moves[left1] == 1) {
                update_row_dir(left_shift[first], left_shift[left1]);
                update_row_dir(left_shift[second], left_shift[left1]);
            }
            else if(possible_moves[left1] == 2) {
                update_row_red(left_shift[first], left_shift[left1]);
                update_row_red(left_shift[second], left_shift[left1]);
            }
            // todo: might also have to be 1
